    ...
    <aside class="filter">
        ...
        <div>
            ...
            <label for="cricket" class="disp-new-line sm-text">
                <input type="radio" id="cricket" name="category"
                    checked={filterState.categoryFilter === "Cricket" } onClick={()=> filterDispatch({type: "CATEGORIES",
                payload: "Cricket"})} />
                Cricket
            </label>
            <label for="football" class="disp-new-line sm-text">
                <input type="radio" id="football" name="category"
                    checked={filterState.categoryFilter === "Football" } onClick={()=> filterDispatch({type: "CATEGORIES", payload: "Football"
                })}/>
                Football
            </label>
            <label for="hockey" class="disp-new-line sm-text">
                <input type="radio" id="hockey" name="category"
                checked={filterState.categoryFilter === "Hockey" } onClick={()=> filterDispatch({type: "CATEGORIES", payload: "Hockey"
            })}/>
                Hockey
            </label>
            <label for="badminton" class="disp-new-line sm-text">
                <input type="radio" id="badminton" name="category"
                checked={filterState.categoryFilter === "Badminton" } onClick={()=> filterDispatch({type: "CATEGORIES", payload: "Badminton"
            })}/>
                Badminton
            </label>
            <label for="basketball" class="disp-new-line sm-text">
                <input type="radio" id="basketball" name="category" 
                checked={filterState.categoryFilter === "Basketball" } onClick={()=> filterDispatch({type: "CATEGORIES", payload: "Basketball"
            })}/>
                Basketball
            </label>
            <label for="tennis" class="disp-new-line sm-text">
                <input type="radio" id="tennis" name="category" 
                checked={filterState.categoryFilter === "Tennis" } onClick={()=> filterDispatch({type: "CATEGORIES", payload: "Tennis"
            })}/>
                Tennis
            </label>
        </div>
        <div>
            ...
            <label for="1star" class="disp-new-line sm-text">
                <input type="radio" id="1star" name="rating" value={filterState.ratingFilter === "1"} 
                onClick = {() => filterDispatch({type: "RATING", payload: 1})}/>
                1 star and above
            </label>
            <label for="2star" class="disp-new-line sm-text">
                <input type="radio" id="2star" name="rating" value = {filterState.ratingFilter === "2"}
                onClick = {() => filterDispatch({type: "RATING", payload: 2})} />
                2 star and above
            </label>
            <label for="3star" class="disp-new-line sm-text">
                <input type="radio" id="3star" name="rating" value = {filterState.ratingFilter === "3"}
                onClick = {() => filterDispatch({type: "RATING", payload: 3})} />
                3 star and above
            </label>
            <label for="4star" class="disp-new-line sm-text">
                <input type="radio" id="4star" name="rating" value = {filterState.ratingFilter === "4"}
                onClick = {() => filterDispatch({type: "RATING", payload: 4})} />
                4 star and above
            </label>
            <label for="4star" class="disp-new-line sm-text">
                <input type="radio" id="5star" name="rating" value = {filterState.ratingFilter === "5"}
                onClick = {() => filterDispatch({type: "RATING", payload: 5})} />
                5 star
            </label>
        </div>
        <div>
            ...
            <label for="l-to-h" class="disp-new-line sm-text">
                ...
                "SORT_BY_PRICE", payload: "PRICE_LOW_TO_HIGH"})}/>
                Price : Low to High
            </label>
            <label for="h-to-l" class="disp-new-line sm-text">
                ...
                "SORT_BY_PRICE", payload: "PRICE_HIGH_TO_LOW"})}/>
                Price : High to Low
            </label>
        </div>
    </aside>
